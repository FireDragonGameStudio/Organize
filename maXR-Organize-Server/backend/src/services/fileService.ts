...
export class FileService {
    private static getFilePath(projectId: string, fileType: string): string {
        if (!VALID_TYPES.includes(fileType)) {
            throw new Error('Invalid requirement file type');
        }
        return path.join(DATA_DIR, projectId, `${fileType}.json`);
    }

    static getRequirements(projectId: string, fileType: string): any[] {
    ...
    private static findTargetFileType(projectId: string, reqId: string): string | null {
        for (const ft of VALID_TYPES) {
            const reqs = this.getRequirements(projectId, ft);
            if (reqs.some((r: any) => r.id === reqId)) return ft;
        }
        return null;
    }

    private static syncTraceLinks(projectId: string, reqId: string, oldLinks: string[], newLinks: string[]): void {
        const added = newLinks.filter(l => !oldLinks.includes(l));
        const removed = oldLinks.filter(l => !newLinks.includes(l));

        const updateLinkInTarget = (targetId: string, action: 'add' | 'remove') => {
            ...
            if (fs.existsSync(filePath)) {
                const reqs = JSON.parse(fs.readFileSync(filePath, 'utf8'));
                const targetReq = reqs.find((r: any) => r.id === targetId);
                if (targetReq) {
                    ...
                    if (action === 'add' && !targetReq.traceLinks.includes(reqId)) {
                        ...
                        targetReq.traceLinks = targetReq.traceLinks.filter((id: string) => id !== reqId);
                        this.replaceFile(projectId, targetFileType, reqs);
                    }
                }
            }
    ...
    static replaceFile(projectId: string, fileType: string, data: any[]): void {
        const filePath = this.getFilePath(projectId, fileType);
        fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    }

    static addRequirement(projectId: string, fileType: string, requirement: any): any {
        const requirements = this.getRequirements(projectId, fileType);
        requirement.id = requirement.id || crypto.randomUUID();
        
        if (!requirement.name) {
            let prefix = '';
            if (fileType === 'user') prefix = 'UR-';
            else if (fileType === 'system') prefix = 'SR-';
            else if (fileType === 'design_input') prefix = 'DIR-';
            else if (fileType === 'software') prefix = 'SWR-';
    ...
    static updateRequirement(projectId: string, fileType: string, reqId: string, updatedReq: any): any {
        const requirements = this.getRequirements(projectId, fileType);
    ...
    static deleteRequirement(projectId: string, fileType: string, reqId: string): void {
        const requirements = this.getRequirements(projectId, fileType);
        const index = requirements.findIndex(r => r.id === reqId);
        if (index === -1) {
            throw new Error('Requirement not found');
    ...
    static bulkUpdateRequirements(projectId: string, fileType: string, updates: any[]): any[] {
        const requirements = this.getRequirements(projectId, fileType);
    ...
    static bulkDeleteRequirements(projectId: string, fileType: string, reqIds: string[]): void {
        const requirements = this.getRequirements(projectId, fileType);
    ...
    static changeRequirementType(projectId: string, oldFileType: string, reqId: string, newFileType: string): any {
        if (!VALID_TYPES.includes(newFileType)) throw new Error('Invalid new file type');
        if (oldFileType === newFileType) return this.getRequirements(projectId, oldFileType).find(r => r.id === reqId);
        ...
        let prefix = '';
        let typeVal = '';
        if (newFileType === 'user') { prefix = 'UR-'; typeVal = 'UR'; }
        else if (newFileType === 'system') { prefix = 'SR-'; typeVal = 'SR'; }
        else if (newFileType === 'design_input') { prefix = 'DIR-'; typeVal = 'DIR'; }
        else if (newFileType === 'software') { prefix = 'SWR-'; typeVal = 'SWR'; }
